            ...
            <div className="space-y-3">
              <div>
                <Field
                  label={t("wizardSlug")}
                  value={proposedSlug}
                  onChange={(v) => setProposedSlug(slugify(v))}
                />
                ...
                <div key={i} className="rounded-xl border border-border p-2">
                  <div className="grid grid-cols-6 gap-2">
                    <input
                      value={m.name}
                      onChange={(e) =>
                        setMenu(menu.map((x, j) => (j === i ? { ...x, name: e.target.value } : x)))
                      }
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setMenu(
                          menu.map((x, j) => (j === i ? { ...x, category: e.target.value } : x)),
                        )
                      }
              ...
              <button
                onClick={() =>
                  setMenu([
                    ...
                    {
                      name: "",
                      category: menu[menu.length - 1]?.category || "Menu",
                      price_cents: 0,
                    },
                  ])
                }
            ...
            <div className="space-y-3">
              ...
              <label className="block text-xs font-semibold">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setSlots({ ...slots, capacity: Math.max(1, parseInt(e.target.value) || 1) })
                  }
                  className="mt-1 w-full rounded-lg border border-border bg-background px-2 py-1.5 text-sm"
                />
              </label>
        ...
        {step !== 1 && !done && (
          <div className="mt-5 flex gap-2">
            ...
            {step < totalSteps ? (
              <button
                onClick={next}
                className="flex-1 rounded-2xl bg-foreground px-4 py-3 text-sm font-black text-background"
              >
                {t("wizardNext")}
              </button>
            ) : (
              <button
                onClick={submit}
                disabled={busy}
                className="flex-1 rounded-2xl bg-foreground px-4 py-3 text-sm font-black text-background disabled:opacity-50"
              >
